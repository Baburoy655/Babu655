...
def file():
    ...
    with tred(max_workers=30) as LIPU:
        fresh()
        tl = str(len(fo))
        ...
        for user in fo:
            ...
            passlist = plist
            if lipu_k =='1':
                lipu_k.submit(api1,ids,names,passlist)
            if lipu_k =='2':
                lipu_k.submit(api2,ids,names,passlist)
            if lipu_k =='3':
                lipu_k.submit(api3,ids,names,passlist)
#-----------------------[ FILE-METHOD-CODE ]-----------------------#
def uaab():
